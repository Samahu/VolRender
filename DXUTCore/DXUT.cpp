...
protected:
    struct STATE
    {
		...
		LPDXUTCALLBACKSHUTDOWN m_ShutdownFunc;				// Shutdown the application callback (happens after destroying device objects)
		...
		void* m_ShutdownFuncUserContext;            // user context for Direct3D device removed callback
...
public:
    ...
    GET_SET_ACCESSOR( LPDXUTCALLBACKSHUTDOWN, ShutdownFunc );
	...
	GET_SET_ACCESSOR( void*, ShutdownFuncUserContext );
...
void WINAPI DXUTSetCallbackShutdown( LPDXUTCALLBACKSHUTDOWN pCallback, void* pUserContext )									   { GetDXUTState().SetShutdownFunc( pCallback ); GetDXUTState().SetShutdownFuncUserContext( pUserContext ); }
...
void WINAPI DXUTShutdown( int nExitCode )
{
	...
	LPDXUTCALLBACKSHUTDOWN pShutdownFunc = GetDXUTState().GetShutdownFunc();
    if( pShutdownFunc )
        pShutdownFunc( GetDXUTState().GetShutdownFuncUserContext() );
